...
class my_PCSA():
    ...
    def evolve(self,pop):
        ...
        for t in range(0, self.__gen):
            ...
            # Tournament Size ----------------------------------
            K = round(K_max - t * ((K_max - K_min)/self.__gen))
            I = numpy.argsort(fitness, axis=0)              # Returns the indices that would sort an array.
            Best_K = numpy.reshape(I[0:K],K)                # Select K of the best crows in the population
            #---------------------------------------------------
            for i in range(0, PopSize):
                
                r=random.random()
                #------------------------------------                
                # Adaptive Priority Based Selection Selection
                num = numpy.random.choice(Best_K,1) #Choose one of the K crows randomly
                #------------------------------------

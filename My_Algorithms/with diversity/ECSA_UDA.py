...
class my_ECSA():
    """
    Enhanced Crow Search Algorithm (ECSA)
    ...
    def evolve(self,pop):
        ...
        for t in range(0, self.__gen):
            ...
            #AP = 1 - (t / self.__gen)
            #AP = 0.05 + (t / self.__gen) # Linear
            #AP = 0.05 * pow(2 , t/self.__gen) #exponential
            # fl = 2 * (1 - (t / self.__gen)) linear
            #fl = 2.0 * pow(0.15, math.sqrt(t)/self.__gen) #exponential
            ...
            for i in range(0, PopSize):
                ...
                if r >= AP: 
                    #xnew[i,:]= xn[i,:]+fl*r*(mem[num,:]-xn[i,:])# Generation of a new position for crow i (state 1)
                    xnew[i,:]= xn[i,:]+fl*r*(mem[num,:]-xn[i,:])# Generation of a new position for crow i (state 1)
                else:                       # Generation of a new position for crow i (state 2)
                    #Random movement
                    #xnew[i,:]= numpy.random.uniform(0, 1, dim) * (ub - lb) + lb
                    xnew[i,:] = (gBest - xn.mean(0)) - random.random() * (
